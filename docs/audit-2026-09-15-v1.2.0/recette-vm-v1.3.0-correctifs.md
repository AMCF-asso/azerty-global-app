...
| 8 | Un premier plan **inconnu** — une fenêtre dont le PID existe mais dont le nom reste inaccessible. Presser `Ctrl+Maj+W`. | **Rien ne se passe** — `UnknownForeground` est une suspension de sécurité au même titre. ⛔ **Ne pas prendre une fenêtre élevée pour ce cas** (R14, revue du 2026-09-21) : `OpenProcess(QUERY_LIMITED_INFORMATION)` réussit à travers l'élévation, le nom du processus est donc lu et le mode résolu vaut `Default`, pas `UnknownForeground`. C'est le comportement voulu — VM-13 du 19/09 a mesuré que le remappage traverse une fenêtre élevée, et suspendre y casserait la frappe dans un terminal administrateur. Seul `EnumProcessModules` échoue en silence, ce qui prive la détection automatique de jeu de sa voie « modules » sur un processus élevé ; la détection par nom, elle, tient. Pour jouer ce geste, il faut une fenêtre réellement opaque, pas une simple élévation. |
