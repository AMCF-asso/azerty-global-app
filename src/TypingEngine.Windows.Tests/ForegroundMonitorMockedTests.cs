...
/// </summary>
public class ForegroundMonitorMockedTests
{
    private readonly TestWindowsTypingHost _host = new();
    ...
    {
        _host.SetCompatibilityOverride("MyApp.exe", "forceOn");
    ...
    public void OverrideForceOn_OnAntiCheat_IsOverridden_ToDisabled()
    {
        ...
        _host.SetCompatibilityOverride("VALORANT.exe", "forceOn");
    ...
    {
        _host.SetCompatibilityOverride("Notepad.exe", "forceOff");
        var mock = SetupMock("Notepad.exe", @"C:\Windows\notepad.exe");
        using var fm = new ForegroundMonitor(mock, IntPtr.Zero, _host);
        Assert.Equal(CompatibilityMode.DisabledAntiCheat, fm.CurrentMode);
    }
    ...
    public void Dispose_CallsUnhookWinEvent()
    {
        ...
        var fm = new ForegroundMonitor(mock, IntPtr.Zero, _host);
        fm.Dispose();
