...
internal enum KeyboardRenderProfile
{
    Full,
    Onboarding,
    // La fenetre Clavier virtuel emploie Full depuis CH4b : Antoine a retenu la carte
    // des trois couches le 2026-09-07, contre un profil a glyphe unique qui a ete
    // rendu, compare, puis supprime.
    Lesson
}
...
internal sealed class KeyboardRenderState
{
    ...
    /// « enfoncée » d'avant CH4a ; sinon la table B du plan §6 colore la touche.
    /// </summary>
...
internal static class KeyboardRenderer
{
        ...
        IntPtr hFontContext)
    {
        ...
        // S4-3 (table B, 2026-09-02) : quand l'état nomme le rôle de la frappe attendue, la table
        // du plan §6 colore la touche ; sans rôle, la peinture « enfoncée » reste.
        if (highlighted && !pressed && !modifierActive && !disabledBackspace && state.HighlightRole != KeyHighlight.None)
            paint = KeyboardTheme.HighlightPaint(state.HighlightRole, palette);
        ...
        bool alignTop = false)
    {
        ...
        // S4-2 (2026-09-02) : la grande police suit la couche active, jamais la position — le
        // caractère qui sera tapé est toujours le plus gros (levier mesuré au plan §18 : masse
        // relative 0,41 → 3,51 quand AltGr est tenu). useMainFont ne nomme plus que l'emplacement
        // d'origine du glyphe ; il ne choisit plus la police.
        IntPtr hFont = isActive ? hFontMain : hFontSmall;
        var oldFont = Win32.SelectObject(hdc, hFont);
