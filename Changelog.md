...
- ⚠️ **Ce que cette garde ne voit pas, et ne peut pas voir.** Elle attrape les refus que Windows signale — bureau sécurisé, session verrouillée, lot malformé. Elle **ne voit pas** un blocage UIPI par une fenêtre plus élevée : la documentation de `SendInput` indique qu’un tel blocage n’est signalé ni par la valeur de retour ni par `GetLastError`, et aucune API ne l’expose. Un compteur de pertes à zéro ne prouve donc pas qu’aucune frappe n’a été perdue face à une fenêtre élevée. Mesuré en VM le 2026-09-19 (VM-13) : sur ce poste, le remappage **traverse** une fenêtre élevée — ce qui rend la perte improbable sans la rendre détectable.
