...
- Défi du jour masqué jusqu’en 1.4.0, où il reviendra revu (décision d’Antoine du 2026-09-24) : plus d’entrée dans « Apprendre ▸ », qui garde Leçons et Revoir l’accueil ; plus de module dans les Leçons ; plus d’annonce unique ni de rappel d’entraînement, ce rappel n’ouvrant que le Défi ; plus de case d’opt-in dans l’accueil ni dans les Paramètres ; plus de section dans « Mes statistiques ». Un seul interrupteur, `DailyChallenge.Enabled`. Le code reste ; les données enregistrées (records, séquence, opt-in, annonce) ne sont ni effacées ni migrées. Témoins : `DefiDuJourMasqueTests`.
- Sollicitation d’avis : jamais pendant une séance de Leçons ou le tutoriel de l’accueil, ni dans les 10 minutes qui suivent sa fermeture (décision d’Antoine du 2026-09-24). Ensuite, il faut une frappe hors exercice suivie des 15 s de silence. Les frappes des Leçons comptent toujours dans les statistiques et peuvent armer le seuil des 20 caractères enrichis, mais ne suffisent plus à déclencher la demande. La même garde vaut pour le partage d’un résultat. Témoins : `AvisApresSeanceTests`.
...
- Le menu S2 mesurait dix-huit lignes, jugées encore trop longues en VM. « Apprendre ▸ » replie Leçons, Défi du jour et Revoir l’accueil ; « À propos et aide ▸ » replie Confidentialité et sécurité, Ressources, Retours et soutien, Noter sur le Store et À propos. Dix-huit lignes deviennent douze, **rien n’est retiré**. Le Défi du jour perd la visibilité immédiate que lui donnait la décision du 2026-08-16 : arbitrage assumé. Défi masqué en 1.3.0 : le sous-menu livré garde Leçons et Revoir l’accueil (voir « Correctifs de l’audit du 24 septembre »).
- ⚠️ Aucun test ne verrouille la structure du menu — `ShowContextMenu` appelle Win32 directement. La vérification est visuelle, en VM. Depuis le 2026-09-24, seules les entrées de « Apprendre ▸ » suivent une liste testée (`LearnMenuEntries`).
...
- La priorité de la sollicitation sur le rappel du Défi du jour **survit désormais au redémarrage** : le champ en mémoire qui la portait est mort, l’état se relit sur disque. Sans ce correctif, un redémarrage laissait le rappel d’entraînement passer devant la sollicitation le même soir. Garde inerte en 1.3.0, le Défi et son rappel étant masqués (voir « Correctifs de l’audit du 24 septembre »).
