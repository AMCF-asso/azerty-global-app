...
- Deux formes distinguées là où le code n'en avait qu'une : `DisplayName` « AZERTY Global », ce que l'utilisateur lit, et `Namespace` « AZERTYGlobal », qui est déjà le `RootNamespace` du csproj, l'`Identity Name` du MSIX, l'`Application Id` et le préfixe du `TaskId` de démarrage. `ConfigFolderName` reste un littéral distinct à dessein : renommer le produit ne doit pas déplacer la configuration et la progression de tout le monde.
